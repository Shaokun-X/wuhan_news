...
from wuhan.definitions import CONFIG_DIR
...
class ReportPipeline:
    def __init__(self):
        ...
        with open(os.path.join(CONFIG_DIR, 'postgres.json'), 'r', encoding='utf-8') as fp:
            self.db_config = json.load(fp)
        ...
        self.unique_violation_counter = 0

    def process_item(self, item, spider):
        item['datetime'] = self.__convert_to_utc(item['datetime'])
        table_name = self.db_config['tbname']
        sql = 'INSERT INTO %s (%s) VALUES (%%(%s)s );' % (table_name, ', '.join(item), ')s, %('.join(item))
        try:
            self.cur.execute(sql, dict(item))
            self.conn.commit()
        except UniqueViolation as e:
            self.unique_violation_counter += 1
            self.logger.info(e)
        return item

    def close_spider(self, spider):
        print(f"there were {self.unique_violation_counter} times unique violation.")
        self.cur.close()
        self.conn.close()

    def __convert_to_utc(self, naive_time):
        local_dt = TZ.localize(naive_time)
